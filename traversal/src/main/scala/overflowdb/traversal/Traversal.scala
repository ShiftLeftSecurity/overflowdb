    ...
    case _ =>
      iterator.flatMap { (a: A) =>
        on(Iterator.single(a)).nextOption match {
          ...
          case Some(branchon) =>
            options.applyOrElse(branchon, (inapplicable: BranchOn) => ((arg: Traversal[A]) => Iterator.empty[NewEnd]))(Iterator.single(a))
        }
      }
  }
...
class TraversalTrackingExt[A](val iterator: Iterator[A]) extends AnyVal {
  ...
  def path: Traversal[Vector[Any]] = iterator.asInstanceOf[PathAwareTraversal[A]].wrapped.map { case (a, p) => p.appended(a) } // fixme: I think ClassCastException is the correct result when the user forgot to enable path tracking. But a better error message to go along with it would be nice.
...
class RepeatTraversalExt[A](val trav: Iterator[A]) extends AnyVal {
  ...
  def repeat[B >: A](
                    ...
                    )(implicit behaviourBuilder: RepeatBehaviour.Builder[B] => RepeatBehaviour.Builder[B] = RepeatBehaviour.noop[B] _): Traversal[B] = {
    val behaviour = behaviourBuilder(new RepeatBehaviour.Builder[B]).build
    val _repeatTraversal =
      repeatTraversal.asInstanceOf[Traversal[B] => Traversal[B]] // this cast usually :tm: safe, because `B` is a supertype of `A`
    trav match {
      case tracked: PathAwareTraversal[A] =>
        val step = PathAwareRepeatStep(_repeatTraversal, behaviour)
        new PathAwareTraversal(tracked.wrapped.flatMap { case (a, p) => step.apply(a).wrapped.map { case (aa, pp) => (aa, p ++ pp) } })
