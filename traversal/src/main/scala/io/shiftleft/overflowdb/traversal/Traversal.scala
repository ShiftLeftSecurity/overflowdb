...
class Traversal[A](elements: IterableOnce[A]) extends IterableOnce[A]
  ...
  def repeat(
              ...
              behaviourBuilder: RepeatBehaviour.Builder[A] => RepeatBehaviour.Builder[A] = identity): Traversal[A] = {
  ...
  private def _repeat(
    ...
    if (this.isEmpty) {
      ...
      val x =
        if (behaviour.emitEverything) this.sideEffect(emitSack.addOne(_))
        else this
      repeatTraversal(x)._repeat(repeatTraversal, behaviour, emitSack)
    }
